...
$innerhtml = '<div class="titlediv">'.$title.'</div><div class="leftdiv">';
foreach($content as $v){
	if($v == 'zclx'){
		$sql = "select a.$bianma as bima,zclx.name as zhi from $zz as a left join zclx on a.zclx=zclx.id where a.id=$id";
	}elseif($v == 'zczt'){
		$sql = "select a.$bianma as bima,zhuangtai.name as zhi from $zz as a left join zhuangtai on a.zczt=zhuangtai.id where a.id=$id";
	}elseif($v == 'bm'){
		$sql = "select a.$bianma as bima,danwei.name as zhi from $zz as a left join danwei on a.bm=danwei.id where a.id=$id";
	}else{
		$sql = "select a.$bianma as bima,a.$v as zhi from $zz as a where a.id=$id";
	}
	//echo $sql.'<br>';
	$requ = mysqli_query($con,$sql);
	$rs = mysqli_fetch_array($requ);
